...
- **A single `allReady`/`isReady` flag that ANDs N independent section queries (the "reveal everything together" gate).** This is the forbidden cascade from the "Page composition" rule, spelled out: gate each section on ONLY the queries it renders, and a warm-cache section MUST NEVER wait on a cold sibling query. The "no staggered wave" instinct (one unified gate so cards reveal in unison) overshoots into "every section is now as slow as the single slowest endpoint." Per-card sub-data (e.g. campaign stats inside a feature card) gets its OWN skeleton gated on its OWN query — never folded into the section gate. Incident 2026-06-01 (distribute.you#1257): brand-overview `allReady` ANDed 8 queries (brand, campaigns, batchStats, 5× outcome counts); feature names live in the `["features"]` cache (`staleTime: 5min`) and paint instantly in the sidebar, but the body held them hostage behind the slowest outcome endpoint → cards showed skeletons for seconds. Fix: render the features grid from the warm cache immediately, scope the outcomes-count gate to the 5 outcome queries only, give per-card stats their own skeleton. (DIS-96 introduced the mega-gate under the "no staggered wave" banner; this is the corrective.)
