...
- **Group at the finest COHERENT level.** Body cards of a feature page = one group; the sidebar's Outcomes count badges = another group; per-card sub-stats = their own finest-grain group. Never widen a group to span a warm + cold pair (see the forbidden-patterns rule above).
...
First landed 2026-06-01 (distribute.you, `sales-cold-email-outreach` feature page + `FeatureLevelSidebar` badges): replaced the page's un-latched `allReady` AND-gate and gave the sidebar entity badges a single latched reveal so all counts appear together instead of popping in one-by-one. Rollout target: migrate brand-overview's inline `outcomesReady` barrier (#1257) onto the primitive to gain the latch, then every other multi-query dashboard page.
