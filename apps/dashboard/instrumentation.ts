...
export async function register() {
  ...
  // Register platform keys, prompts, and chat config
  await registerPlatformKeys();
  await registerPlatformPrompts();
  await registerPlatformChatConfig();
}

// ── Platform key registration → key-service ──
...
async function registerPlatformKeys() {
  const url = process.env.KEY_SERVICE_URL;
  const apiKey = process.env.KEY_SERVICE_API_KEY;

  if (!url || !apiKey) {
    console.warn("[instrumentation] KEY_SERVICE_URL or KEY_SERVICE_API_KEY not set, skipping platform key registration");
    return;
  }

  // Validate all env vars are present before registering any
  const missing = PLATFORM_KEY_PROVIDERS.filter((p) => !process.env[p.envVar]);
  if (missing.length > 0) {
    throw new Error(
      `[instrumentation] Missing platform key env vars: ${missing.map((p) => p.envVar).join(", ")}`
    );
  }

  for (const { provider, envVar } of PLATFORM_KEY_PROVIDERS) {
    try {
      const res = await fetch(`${url}/platform-keys`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-API-Key": apiKey,
      ...
      throw new Error(`[instrumentation] Failed to register platform key "${provider}": ${err}`);
    }
  }

  console.log(`[instrumentation] Registered ${PLATFORM_KEY_PROVIDERS.length} platform keys`);
}

// ── Platform prompts → content-generation-service ──
...
async function registerPlatformPrompts() {
  const url = process.env.CONTENT_GENERATION_SERVICE_URL;
  const apiKey = process.env.CONTENT_GENERATION_SERVICE_API_KEY;

  if (!url || !apiKey) {
    console.warn("[instrumentation] CONTENT_GENERATION_SERVICE_URL or API_KEY not set, skipping prompt registration");
    return;
  }

  const prompt = COLD_EMAIL_PROMPT.replace("{date}", new Date().toISOString().split("T")[0]);

  try {
    const res = await fetch(`${url}/platform-prompts`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "X-API-Key": apiKey,
    ...
    console.log("[instrumentation] Registered platform prompt: cold-email");
  } catch (err) {
    throw new Error(`[instrumentation] Failed to register platform prompts: ${err}`);
  }
}

// ── Platform chat config → chat-service ──

async function registerPlatformChatConfig() {
  const url = process.env.CHAT_SERVICE_URL;
  const apiKey = process.env.CHAT_SERVICE_API_KEY;

  if (!url || !apiKey) {
    console.warn("[instrumentation] CHAT_SERVICE_URL or API_KEY not set, skipping chat config registration");
    return;
  }

  try {
    const res = await fetch(`${url}/platform-config`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "X-API-Key": apiKey,
      ...
      body: JSON.stringify({
        systemPrompt: "You are a helpful assistant embedded in a workflow management dashboard. You help users understand their workflows — what each step does, how the DAG is structured, what inputs and outputs are expected, and how to troubleshoot issues. Be concise and practical. When referencing workflow steps, use their names.",
      }),
    });

    if (!res.ok) {
      const body = await res.text().catch(() => "");
      console.warn(`[instrumentation] Chat config registration failed: ${res.status} ${body}`);
    } else {
      console.log("[instrumentation] Registered platform chat config");
    }
  } catch (err) {
    console.warn("[instrumentation] Chat config registration error:", err);
  }
}
