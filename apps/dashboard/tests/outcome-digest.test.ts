...
describe("daily outcome digest", () => {
  ...
  it("prepares a per-brand send for EVERY customer user (no beta gate)", async () => {
    ...
    const fetchMock: DigestFetch = async (input, init) => {
      ...
      if (url === "https://api.example.test/v1/features/sales-cold-email-outreach/revenue?brandId=brand_1&pricing=net&leads=full") {
  ...
  it("does not prepare sends when a brand has no outcome organizations", async () => {
    const fetchMock: DigestFetch = async (input) => {
      ...
      if (url === "https://api.example.test/v1/features/sales-cold-email-outreach/revenue?brandId=brand_empty&pricing=net&leads=full") {
        return jsonResponse({
          featureSlug: "sales-cold-email-outreach",
  ...
  it("isolates a failing brand — one brand's fetch error does not skip the rest", async () => {
    ...
    const fetchMock: DigestFetch = async (input) => {
      ...
      if (url === "https://api.example.test/v1/features/sales-cold-email-outreach/revenue?brandId=brand_bad&pricing=net&leads=full") {
        // brand_bad's revenue keeps timing out — must NOT abort the whole run.
        throw Object.assign(new Error("The operation was aborted due to timeout"), { name: "TimeoutError" });
      }
      if (url === "https://api.example.test/v1/features/sales-cold-email-outreach/revenue?brandId=brand_good&pricing=net&leads=full") {
        return jsonResponse(brandRevenue(3, day));
      }
      throw new Error(`Unexpected fetch ${url}`);
    };
