...
describe("chat config registration at cold start (via API service)", () => {
  ...
  it("should register chat config via PUT /platform-chat/config on API service", () => {
  ...
  it("should include a system prompt for the chat config", () => {
    const content = fs.readFileSync(instrumentationPath, "utf-8");
    expect(content).toContain("systemPrompt");
    expect(content).toContain("CHAT_SYSTEM_PROMPT");
  });
});
