...
describe("isPersistableQueryKey — STATUS-AGNOSTIC predicate (the per-query persister gate)", () => {
  ...
  // no-op (every load cold-fetches the 30s Neon path). So it MUST ignore status.
...
describe("query-provider wiring — local-first per-query persisted cache", () => {
  ...
  it("no-ops the persister storage while orgId is null / on the server (no anon bleed)", () => {
    ...
    expect(src).toContain("persistEnabled ? makeIdbStorage(persisterStorageKey(orgId)) : undefined");
  });
...
describe("cache retention durations", () => {
  it("maxAge is 30 days — long enough to stay instant, finite so the store is bounded", () => {
    ...
    // response this browser had ever received, across every org — which the boot-time
