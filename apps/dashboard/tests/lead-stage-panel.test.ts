...
/**
 * The `<LeadStageSection …/>` element, bounded by its OWN close rather than by a
...
const stageSectionCall = () => {
  const at = PAGE.indexOf("<LeadStageSection");
...
describe("lead stage panel", () => {
  it("keys the control set on the campaign's LEG, never on a goal", () => {
    // A goal cannot say whether the campaign starts at a reply or at a website visit,
    // the exact distinction this panel records. The campaign states its own leg.
    const slice = PAGE.slice(PAGE.indexOf("const panelLeg ="), PAGE.indexOf("const onSetStage ="));
    expect(slice).toContain("activeLegs[0]");
    expect(slice).toContain("leadLegStages(panelLeg)");
    expect(slice).not.toContain("optimizationGoal");
    expect(slice).not.toContain("stepsFor(");
  });

  it("mounts wherever a leg is STATED, and states nothing where several run", () => {
    // Only a campaign states one leg. Brand and offer run several at once, so the
    // section does not render there at all.
    expect(PAGE).toContain("{panelLeg && (");
    expect(PAGE).toContain("const panelLeg = campaignId ? (activeLegs[0] ?? null) : null;");
  });
...
describe("the panel answers the click at once", () => {
  ...
  it("still re-reads the steps from the producer, never deriving it here", () => {
  ...
  it("leads the panel with what happened, above who the person works for", () => {
    ...
    // scroll to.
    expect(PAGE.indexOf("<LeadStageSection")).toBeLessThan(PAGE.indexOf(">Organization<"));
  });

  it("states the delivery badge once, on the stage row that owns it", () => {
...
describe("stating what a won deal was worth", () => {
  ...
  it("reads the recorded amount back where it was typed", () => {
    expect(SECTION).toContain('data-testid="lead-stage-value"');
  ...
  it("states a CRM-evidenced won deal's value, not only a hand-stated one", () => {
    ...
    const crm = SECTION.slice(
      ...
      SECTION.indexOf('data-testid="lead-stage-crm"'),
    );
    expect(crm).toContain('data-testid="lead-stage-value"');
...
describe("stating what the step cost the customer", () => {
  ...
  it("puts the recorded cost on screen, and the page passes it", () => {
    ...
    expect(SECTION).toContain('data-testid="lead-stage-cost"');
...
describe("the steps constrain their neighbours", () => {
  ...
  it("reads implied from the producer's own origin, never re-deriving the order here", () => {
  ...
  it("states what our sending did ABOVE the steps, read-only, and the page passes it", () => {
...
describe("the panel walks the campaign's OWN leg", () => {
  it("walks the leg's own stages and no wider population", () => {
    expect(PAGE).toContain("leadLegStages(panelLeg)");
    expect(PAGE).not.toContain("leadFunnelStages(");
  });

  it("names the later stages a Won't-happen also ends, before it is clicked", () => {
    expect(SECTION).toContain("stages.slice(stageIndex + 1)");
  });
});

describe("the statements and the reply picker follow the leg", () => {
  it("reads the statements wherever the section renders", () => {
    // A statement is keyed on the leads_campaigns row the table already carries.
    const slice = PAGE.slice(PAGE.indexOf("useLeadStepStatements("), PAGE.indexOf("const setStage ="));
    expect(slice).toContain("panelLeg && selectedLead");
  });
