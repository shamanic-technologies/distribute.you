...
describe("launchDestinationHref", () => {
  it("names the offer outright, with NO marker: it is the end of the walk", () => {
    expect(launchDestinationHref({ orgId: "org_1", brandId: "b1", offerId: "o1" })).toBe(
      "/orgs/org_1/brands/b1/offers/o1",
    );
  });
  ...
  it("degrades to the plain sign-in landing when it cannot name the offer", () => {
    ...
    expect(launchDestinationHref({ orgId: "org_1", brandId: "b1", offerId: null })).toBe(
      "/orgs/org_1/brands/b1?land=1",
    );
  });

  it("escapes every segment it is given", () => {
    expect(launchDestinationHref({ orgId: "org/1", brandId: "b 1", offerId: "o/1" })).toBe(
      "/orgs/org%2F1/brands/b%201/offers/o%2F1",
    );
  });
});

// The component has to PASS the scope, not merely be able to: a redirect perfectly
// capable of naming an offer lands on the brand page for good if the launch never
// hands it one.
describe("the onboarding terminal redirect", () => {
  it("routes through the shared destination helper", () => {
    ...
    // The funnel is no longer a page, so the redirect names no funnel.
    expect(ONBOARDING).not.toContain("funnelKey: result.funnelKey");
  });
