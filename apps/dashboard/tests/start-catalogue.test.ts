...
describe("funnelsForChannels", () => {
  it("offers only funnels a picked channel can actually sell", () => {
    const keys = funnelsForChannels([COLD_EMAIL], CONVO_PICK).map((f) => f.key);
    expect(keys).toEqual(["sales_meetings_from_conversation"]);
  });

  it("offers nothing when no picked channel sells a funnel", () => {
    expect(funnelsForChannels([META_LEAD_ADS], ["in_ad_form_submission"])).toEqual([]);
  });
  ...
  it("offers only the funnels that START on a picked outcome, not every funnel a channel sells", () => {
    ...
    expect(funnelsForChannels([coldEmailProd], CONVO_PICK).map((f) => f.key)).toEqual([
      "sales_meetings_from_conversation",
    ]);
    expect(funnelsForChannels([coldEmailProd], VISIT_PICK).map((f) => f.key).sort()).toEqual([
      "sales_meetings_from_website",
      "website_purchases",
    ]);
    expect(funnelsForChannels([coldEmailProd], [...CONVO_PICK, ...VISIT_PICK])).toHaveLength(3);
  });

  it("offers nothing on an outcome no funnel starts on, rather than a funnel that starts elsewhere", () => {
    expect(funnelsForChannels([GOOGLE_ADS], CONVO_PICK)).toEqual([]);
  });
  ...
  it("takes the LONGEST run length across the channels selling it", () => {
    ...
    const purchase = funnelsForChannels([GOOGLE_ADS], VISIT_PICK).find((f) => f.key === "website_purchases");
  ...
  it("sums the day rate, because funding a funnel funds every channel picked for it", () => {
    const meeting = funnelsForChannels([GOOGLE_ADS, CLOSER], VISIT_PICK).find(
  ...
  it("lists the cheapest day rate first", () => {
    const f = funnelsForChannels([GOOGLE_ADS, CLOSER], VISIT_PICK).find((x) => x.key === "sales_meetings_from_website");
    expect(f!.channelSlugs[0]).toBe("founder-led-closing");
  });
});

describe("funnelReachesOutcome", () => {
  it("matches a funnel on the step it starts on", () => {
    expect(funnelReachesOutcome("sales_meetings_from_conversation", CONVO_PICK)).toBe(true);
    expect(funnelReachesOutcome("sales_meetings_from_conversation", VISIT_PICK)).toBe(false);
    expect(funnelReachesOutcome("form_magnet", VISIT_PICK)).toBe(true);
  });

  // An in-ad outcome starts no funnel; it lands on a rung inside one.
  it("matches an in-ad outcome on the rung it lands on", () => {
    expect(funnelReachesOutcome("form_magnet", ["in_ad_form_submission"])).toBe(true);
    expect(funnelReachesOutcome("website_purchases", ["in_ad_form_submission"])).toBe(false);
    expect(funnelReachesOutcome("sales_meetings_from_website", ["in_ad_booked_meeting"])).toBe(true);
    expect(funnelReachesOutcome("sales_meetings_from_conversation", ["in_ad_booked_meeting"])).toBe(true);
    expect(funnelReachesOutcome("form_magnet", ["in_ad_booked_meeting"])).toBe(false);
  });

  it("reads the pre-rename spelling too", () => {
    expect(funnelReachesOutcome("reply_meeting", CONVO_PICK)).toBe(true);
  });

  it("throws on a funnel key this app cannot name, never guesses", () => {
    expect(() => funnelReachesOutcome("brand_new_funnel", VISIT_PICK)).toThrow();
  });
});
