...
describe("bucket key bounds", () => {
  it("brackets a bucket's own keys", () => {
    const [lower, upper] = bucketKeyBounds("distribute-dashboard-cache:platform");
    const key = "distribute-dashboard-cache:platform-abc123";
    expect(key >= lower && key <= upper).toBe(true);
  });

  it("excludes another bucket's keys", () => {
    const [lower, upper] = bucketKeyBounds("distribute-dashboard-cache:org_A");
    const other = "distribute-dashboard-cache:org_B-abc123";
  ...
  it("excludes a bucket whose name merely EXTENDS this one", () => {
    ...
    const [lower, upper] = bucketKeyBounds("distribute-dashboard-cache:platform");
    const sibling = "distribute-dashboard-cache:platform2-abc123";
...
describe("bucketEntries", () => {
  const store = () =>
    fakeStore({
      "distribute-dashboard-cache:platform-aaa": snapshot("platform-aaa", NOW),
      "distribute-dashboard-cache:platform-bbb": snapshot("platform-bbb", NOW),
      "distribute-dashboard-cache:org_A-ccc": snapshot("org_A-ccc", NOW),
      "distribute-dashboard-cache:org_B-ddd": snapshot("org_B-ddd", NOW),
    });

  it("returns exactly this bucket's entries", async () => {
    const { use } = store();
    const entries = await bucketEntries("distribute-dashboard-cache:platform", {
      store: use,
      range: (lo, hi) => ({ lo, hi }) as unknown as IDBKeyRange,
    });
    expect(entries.map(([k]) => k)).toEqual([
      "distribute-dashboard-cache:platform-aaa",
      "distribute-dashboard-cache:platform-bbb",
    ]);
  });

  it("never READS another bucket's values — the memory claim, not just the filter", async () => {
    const { use, readKeys } = store();
    await bucketEntries("distribute-dashboard-cache:platform", {
      store: use,
      range: (lo, hi) => ({ lo, hi }) as unknown as IDBKeyRange,
    });
  ...
  it("pairs each key with its own value", async () => {
    const { use } = store();
    const entries = await bucketEntries("distribute-dashboard-cache:org_A", {
      store: use,
      range: (lo, hi) => ({ lo, hi }) as unknown as IDBKeyRange,
    });
    expect(entries).toHaveLength(1);
    const [key, value] = entries[0];
    expect(key).toBe("distribute-dashboard-cache:org_A-ccc");
...
describe("sweepStaleEntries", () => {
  it("deletes stale entries across EVERY bucket and keeps the fresh ones", async () => {
    const { use, data } = fakeStore({
      "distribute-dashboard-cache:platform-fresh": snapshot("fresh", NOW - DAY),
      "distribute-dashboard-cache:platform-old": snapshot("old", NOW - 40 * DAY),
      "distribute-dashboard-cache:org_A-old": snapshot("orgA", NOW - 90 * DAY),
      "distribute-dashboard-cache:org_B-busted": snapshot("orgB", NOW, "0"),
    });
    const removed = await sweepStaleEntries("1", NOW, PERSIST_MAX_AGE_MS, {
      store: use,
    });
    expect(removed).toBe(3);
    expect([...data.keys()]).toEqual(["distribute-dashboard-cache:platform-fresh"]);
  });
...
describe("reclaimLegacyStore", () => {
  it("clears the store once and leaves the marker behind", async () => {
    const { use, data } = fakeStore({
      "distribute-dashboard-cache:platform-a": snapshot("a", NOW),
      "distribute-dashboard-cache:org_A-b": snapshot("b", NOW),
    });
  ...
  it("does nothing on a store already reclaimed", async () => {
    ...
    expect(data.has("distribute-dashboard-cache:platform-a")).toBe(true);
  });
});
