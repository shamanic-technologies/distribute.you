 ...
 *
 * The Leads page renders at four grains and each of the three offer-scoped ones
 ...
 *   /offers/:o/funnels/:k/leads             -> /offers/:o/funnels/:k/audiences
 *
 * All three render `CustomerAudiencesPage`, which reads the `?audienceId=` seed
 ...
 * a campaign's Leads page kicked the reader out of the campaign and from a
 * funnel's out of the funnel. `top-audiences-card.tsx` already grain-matches for
 ...
 * belongs to the offer it was assembled for, and a campaign/funnel page under a
...
export function audienceDetailHref({
  ...
  /** The `/funnels/:funnelKey` segment, present only on a funnel route. */
  ...
  const scoped = sameOffer ? scopeSegment(campaignId, funnelKey) : "";
  return `${base}${scoped}/audiences?audienceId=${encodeURIComponent(audienceId)}`;
}

/**
 * The deepest scope the route names, campaign before funnel. The two routes are
 * mutually exclusive in the tree, so the order only decides a case that cannot
 * happen; stating it keeps the result deterministic rather than accidental.
 */
function scopeSegment(campaignId?: string | null, funnelKey?: string | null): string {
  const campaign = nonEmpty(campaignId);
  if (campaign) return `/campaigns/${encodeURIComponent(campaign)}`;
  const funnel = nonEmpty(funnelKey);
  // The funnel route reads its segment back through `decodeURIComponent`, so it
  // is encoded here — a key carrying a slash would otherwise name a deeper path.
  if (funnel) return `/funnels/${encodeURIComponent(funnel)}`;
  return "";
}
