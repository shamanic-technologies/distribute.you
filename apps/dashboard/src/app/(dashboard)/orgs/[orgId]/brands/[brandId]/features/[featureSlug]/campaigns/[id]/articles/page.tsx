...
export default function CampaignArticlesPage() {
  ...
  const journalistMap = useMemo(() => {
    const map = new Map<string, string>();
    for (const j of journalistsData?.journalists ?? []) {
  ...
  // Sort by article published date (newest first), fallback to discovery creation
      ...
      <div className={`${selected ? "hidden md:block md:w-1/2" : "w-full"} p-4 md:p-8 overflow-y-auto transition-all`}>
        <div className="flex items-center justify-between mb-6">
          <h1 className="font-display text-xl font-bold text-gray-800">
            Articles
            <span className="ml-2 text-sm font-normal text-gray-500">({articles.length.toLocaleString("en-US")})</span>
          </h1>
        </div>
