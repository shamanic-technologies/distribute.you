...
import { getBrand, createFeature, ApiError, type Feature, type FeatureInput, type FeatureOutput } from "@/lib/api";
...
export default function CreateFeaturePage() {
  ...
  // Stable chat ID — does not change when transitioning from creation to settings
  ...
  const featureContext = useMemo(() => {
    // Post-creation: settings mode with full feature identity
    if (createdFeature) {
      return {
        ...
        feature: {
          ...
          slug: createdFeature.slug,
          dynastySlug: createdFeature.dynastySlug,
          name: createdFeature.name,
          dynastyName: createdFeature.dynastyName,
          version: createdFeature.version,
          description: createdFeature.description,
          icon: createdFeature.icon,
          category: createdFeature.category,
          channel: createdFeature.channel,
          audienceType: createdFeature.audienceType,
          inputs: createdFeature.inputs.map((inp) => ({
            key: inp.key,
            label: inp.label,
            type: inp.type,
          ...
          outputs: createdFeature.outputs.map((out) => ({
            key: out.key,
            displayOrder: out.displayOrder,
          })),
          charts: createdFeature.charts,
          entities: createdFeature.entities,
        },
        ...
        instructions: [
          ...
          "",
          "== CURRENT FEATURE ==",
          `Name: ${createdFeature.dynastyName ?? createdFeature.name}`,
          `ID: ${createdFeature.id}`,
          `Slug: ${createdFeature.slug}`,
          `Dynasty Slug: ${createdFeature.dynastySlug}`,
          `Dynasty Name: ${createdFeature.dynastyName}`,
          `Version: ${createdFeature.version}`,
          "",
  ...
  }, [createdFeature, brandId, brand, draft, existingFeaturesRef, statsRegistryRef]);
  ...
  }, [isSaving, draft, queryClient, orgId, brandId]);
    ...
    <div className="flex h-full overflow-hidden bg-white dark:bg-gray-900">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="font-display text-[15px] font-bold text-gray-900 dark:text-gray-100 truncate">
                    {createdFeature.dynastyName ?? createdFeature.name}
                  </h1>
                  <p className="text-[10px] text-gray-400 dark:text-gray-500 font-mono truncate mt-0.5">{createdFeature.slug}</p>
                </div>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-5">
              <FeatureOverview feature={createdFeature} registry={registry} />
            </div>
          </>
        ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <FeatureCreatorChat
          ...
          onFeatureUpdate={createdFeature ? undefined : handleFeatureUpdate}
        />
      </div>
    </div>
  );
