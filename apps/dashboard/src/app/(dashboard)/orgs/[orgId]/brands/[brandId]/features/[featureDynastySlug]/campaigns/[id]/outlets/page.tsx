...
function OutletRow({ outlet, costCents, isSelected, onClick }: { outlet: DeduplicatedOutlet; costCents: string | null; isSelected: boolean; onClick: () => void }) {
  ...
  const displayStatus = deriveDisplayStatusFromCounts(outlet.status?.campaign ?? outlet.status?.brand);
  return (
    <button
      type="button"
      onClick={onClick}
...
export default function CampaignOutletsPage() {
  ...
  const groupedByStatus = useMemo(() => {
    ...
    for (const o of outlets) {
      const ds = deriveDisplayStatusFromCounts(o.status?.campaign ?? o.status?.brand);
