        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto min-h-0">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
            ...
            {(lastErrorInfo || (chatError && status === "error")) && !isStreaming && (
              <div className="rounded-xl border border-red-200 dark:border-red-400/20 bg-red-50 dark:bg-red-500/[0.08] p-3 text-sm text-red-700 dark:text-red-300 animate-in fade-in duration-150">
                <p>{lastErrorInfo?.message ?? chatError?.message ?? "An error occurred"}</p>
                {lastErrorInfo && RETRYABLE_CODES.has(lastErrorInfo.code) && (
                  <button
                    type="button"
                    onClick={retryLastMessage}
                    className="mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-red-100 dark:bg-red-500/20 hover:bg-red-200 dark:hover:bg-red-500/30 text-red-700 dark:text-red-200 rounded-lg transition-colors"
                  >
                    <ArrowPathIcon className="w-3.5 h-3.5" />
