...
function TenantMenu({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white py-1 shadow-xl">
      ...
      {/* Org-scoped settings — reached from the ORG entry point, per every
          multi-tenant leader checked (Linear, Notion, Vercel, GitHub). The entry
          reads "Organization Settings" rather than "Billing" because the sidebar
          it opens carries the whole org group (Billing AND API Key) — a label
          narrower than its destination reads as a missing entry for the rest.
          The href stays `/orgs/:id/billing`: Billing is that group's first item,
          and every deep link (billing-guard, credit-alerts, onboarding) uses it. */}
      {t.orgId && (
        <div className="mt-1 border-t border-gray-100 px-1 pt-1">
          ...
          >
            <span className="text-gray-400"><OrgSettingsIcon /></span>
