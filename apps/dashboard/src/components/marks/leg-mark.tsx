...
import { legMarkFor, type LegGlyph } from "@/lib/leg-marks";

// The tile that stands for ONE LEG — the move a campaign actually
...
// (`lib/leg-marks.ts`) stays a plain unit-testable module with no icon import and
...
const LEG_ICONS: Record<LegGlyph, Icon> = {
...
// Byte-equal to the channel mark: `xs` is the 18px offer tile, `sm` the 32px
...
export function LegMark({
  ...
  /** The step the leg moves a lead OUT of — null for a leg onto a step from nothing. */
  fromKey: string | null;
  ...
  const mark = legMarkFor(fromKey, toKey);
