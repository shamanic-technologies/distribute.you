...
function BetaGate({ children }: { children: React.ReactNode }) {
  ...
  if (!isLoaded) return <div className="v2-root h-[100dvh]" />;
  if (!isBetaEmail(user?.primaryEmailAddress?.emailAddress)) {
    return (
      <div className="v2-root flex h-[100dvh] items-center justify-center p-6">
        <div className="k-card max-w-sm p-6 text-center">
