...
// A pipeline: three nodes joined left to right. The workflow IS the pipeline that
// runs the channel, so the mark says "steps that feed each other" rather than a
// document or a gear.
const WorkflowsIcon = () => (
  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="w-5 h-5">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M6 7.5a2.25 2.25 0 100-4.5 2.25 2.25 0 000 4.5zm0 0v9m0 0a2.25 2.25 0 104.5 0 2.25 2.25 0 00-4.5 0zm4.5 0h4.5m0 0a2.25 2.25 0 104.5 0 2.25 2.25 0 00-4.5 0z" />
  </svg>
);
...
function CampaignLevelSidebar({ orgId, brandId, offerId, campaignId, pathname }: {
  ...
  const featureSlug = useSoleFeatureSlug();
  const revenueOk = isRevenueFeature(featureSlug);
  // The Workflows surface is BETA, and the dashboard has no PostHog gating —
  // `useFeatureFlag` returns false for everyone here, so it would hide the entry from
  // staff too. The email allowlist is what actually evaluates, and the badge rides the
  // nav entry (the page's own body gates on the same hook).
  const workflowsOk = useIsBetaUser();
  ...
  const items: SidebarItem[] =
    revenueOk
      ? [
          ...
          // Which workflows this campaign's CHANNEL can run, what each one did for
          // this campaign, and which one is running now.
          ...(workflowsOk
            ? [
                {
                  id: "campaign-workflows",
                  label: "Workflows",
                  href: `${campaignBase}/workflows`,
                  icon: <WorkflowsIcon />,
