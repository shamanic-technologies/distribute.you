...
/**
 * What happened to ONE lead at each step of its campaign's leg, stated by a person
 ...
 * mutation and no knowledge of how an outcome is recorded.
 */
...
export function LeadStageSection({
   ...
   * the steps because every leg starts after the email arrives. Absent (a scope
   * with no delivery evidence to hand) and the row simply does not render.
   */
  delivery?: ReactNode;
  /**
   * Stages the producer CONCLUDED rather than a person stating. Steps are ORDERED, so a
   ...
   * take it back. A tracker-reported outcome and one the producer merely implies are
  ...
  /**
   * The reply row's own control, when the leg has one. A reply is not a yes/no —
   * nine kinds in four groups — so it gets a picker rather than the two buttons every
   * other row carries. Absent and the row reads as before.
   */
  reply?: {
    kind: string | null;
    pending: boolean;
   ...
   * offer's own stated lifetime revenue, resolved by the caller
  ...
  // No stages is the brand-level case: several legs run at once, so there is no single
  // leg to walk this lead through. State nothing rather than render an empty card.
  if (stages.length === 0) return null;

  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 mb-4">
      <h3 className={`${HEADING_CLASS} mb-1`}>Progress</h3>
      <p className="text-xs text-gray-500 mb-3">{legName}</p>
      ...
      <ul className="divide-y divide-gray-100">
        ...
        {stages.map((stage, stageIndex) => {
          ...
          // What ending this step also ends. Steps are ORDERED, so one click here ends
          // every later step too — the control says so before it is pressed rather than
          // after, which is the difference between a decision and a surprise.
          ...
          // never as a dead control. Two stages are like this: a
          ...
          return (
            <li key={stage.key} className="flex items-center justify-between gap-3 py-2 first:pt-0 last:pb-0">
              <span className="text-sm text-gray-800 min-w-0 flex items-center gap-2">
                <StepMark stageKey={stage.key} size="xs" />
                ...
                {isImplied ? (
                  ...
                  <>
                    ...
                    {state === "outcome" && typeof values?.[stage.key] === "number" && (
                      <span
                        ...
                        data-testid="lead-stage-value"
                      >
                        {formatValue(values[stage.key] as number)}
                      </span>
                    )}
                    <span className="text-xs text-gray-500" data-testid="lead-stage-crm">
                      From your CRM
                    </span>
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${
                  ...
                  <>
                    ...
                    <StageButton
                      ...
                      // being a control: a tracker reported it, or the producer concluded
      ...
      {error && (
        <p className="text-xs text-red-600 mt-2" data-testid="lead-stage-error">
          {error}
        </p>
      )}
    </div>
