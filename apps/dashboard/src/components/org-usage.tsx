...
export function OrgUsageSection({ brands, pending = false }: { brands: Brand[]; pending?: boolean }) {
  ...
  const isPending = pending || ((brandGroupsLoading || totalCostLoading) && !hasData);
  ...
  if (!isPending && totalCents === 0) return null;
  ...
  // Always render the legend structure; show placeholder rows while pending with no data.
  const legendRows = isPending && segments.length === 0
    ? Array.from({ length: 4 }, (_, i) => ({ name: `placeholder-${i}`, cents: 0, percentage: 0, color: "#e5e7eb" }))
    : segments;
  const costRows = isPending && totalCostBreakdown.length === 0
    ? Array.from({ length: 6 }, (_, i) => ({ name: `placeholder-${i}`, cents: 0 }))
    : totalCostBreakdown;
      ...
      <div className="flex flex-col sm:flex-row items-center gap-6">
        ...
        <div
          ...
          style={{
            width: 160,
            height: 160,
            background: isPending ? "#e5e7eb" : `conic-gradient(${stops.join(", ")})`,
          }}
        >
          ...
          {legendRows.map((seg) => (
            <div key={seg.name} className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-700 flex-1 truncate">
                {isPending ? <Skeleton className="h-4 w-24" /> : seg.name}
              </span>
              {isPending ? (
