 ...
 *
 * Purple, like the `Learning` tag it belongs to: this is a waiting state, and amber
 * would read as a warning about something the customer did wrong. Every class is in
 * the `html.dark` remapped set (`bg-purple-50` / `text-purple-600` / `text-purple-700` /
 * `border-purple-200`), so it does not paint a light block on the dark surface, and the
 * border runs the full perimeter at 1px per the no-side-accent rule.
 */
  ...
  return (
    <>
      <div className="mb-4 rounded-xl border border-purple-200 bg-purple-50 px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
          ...
          {doubled != null && (
            <button
              ...
              className="text-xs font-medium text-purple-700 underline underline-offset-2 hover:no-underline"
            >
        ...
        <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-purple-200" aria-hidden>
          <div
