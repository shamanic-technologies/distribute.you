...
const STORAGE_PREFIX = "campaign-prefill-chat";
...
interface CampaignPrefillChatProps {
  chatId: string;
  campaignContext: Record<string, unknown>;
  onFieldsUpdate?: (fields: Record<string, string>) => void;
}

export function CampaignPrefillChat({
  chatId,
  campaignContext,
  onFieldsUpdate,
}: CampaignPrefillChatProps) {
      ...
      {hasMessages ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          ...
          <p className="text-xs text-gray-500 text-center max-w-xs leading-relaxed">
            Ask me to refine the pre-filled fields, extract brand information, or adjust campaign details.
          </p>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex items-end gap-2 bg-gray-50 rounded-xl border border-gray-200 pl-3 pr-1.5 py-1.5 focus-within:border-brand-300 focus-within:ring-2 focus-within:ring-brand-500/10 transition-all">
            <textarea
              ...
              placeholder="Refine campaign fields, extract brand info..."
              disabled={isStreaming}
              rows={1}
