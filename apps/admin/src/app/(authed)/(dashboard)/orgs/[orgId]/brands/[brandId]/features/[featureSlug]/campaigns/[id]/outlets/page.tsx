...
export default function CampaignOutletsPage() {
  ...
  const outletStatusEntries = useMemo(
    () => outlets.map((o) => ({ id: o.id, status: deriveDisplayStatusFromCounts(o.status?.campaign ?? o.status?.brand) })),
    [outlets],
  );
  const latchedStatuses = useMonotonicStatuses(outletStatusEntries, STATUS_PRIORITY, "campaign-outlets");
  ...
  const csv = useMemo(
    () => buildOutletCsv(
      outlets,
      (o) => latchedStatuses.get(o.id) ?? deriveDisplayStatusFromCounts(o.status?.campaign ?? o.status?.brand),
  ...
  const groupedByStatus = useMemo(() => {
    ...
    for (const o of outlets) {
      const ds = latchedStatuses.get(o.id) ?? deriveDisplayStatusFromCounts(o.status?.campaign ?? o.status?.brand);
          ...
          <div className="space-y-2">
            {paginatedOutlets.pageItems.map((outlet) => (
              <OutletRow
                key={outlet.id}
                outlet={outlet}
                displayStatus={latchedStatuses.get(outlet.id) ?? deriveDisplayStatusFromCounts(outlet.status?.campaign ?? outlet.status?.brand)}
