  ...
  const workflowContext = useMemo(() => {
    ...
    return {
      ...
      instructions: [
        ...
        "The COMPLETE workflow DAG is NOT inlined here (it can be very large). To read it, call get_workflow_details with the UUID above — do that whenever you need to inspect, explain, or edit the pipeline. You already have the feature definition it implements below.",
        "",
        "== DAG FORMAT (what get_workflow_details returns) ==",
        ...
        "",
        "== SCOPE (STRICT) ==",
        "Your editing scope is EXCLUSIVELY this workflow (UUID above). You MUST NOT modify or delete any other workflow.",
        "However, you CAN and SHOULD read other workflows for reference — especially recent versions or similar workflows — to reuse proven patterns (e.g. template variable mappings, node configurations) rather than inventing solutions from scratch.",
        "If the user asks you to change other workflows, politely decline and explain that your editing scope is limited to the current workflow only.",
