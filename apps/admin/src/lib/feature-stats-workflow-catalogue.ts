...
export function workflowCatalogueInstructions(
  ...
  return [
    ...
    "",
    "== NAMING A MODEL, TEMPLATE OR ANY ENUMERATED VALUE ==",
    "The user names things in product language (\"DeepSeek Flash v4\", \"the Sonnet",
    "one\"); the service accepts an exact token, and the two are rarely the same",
    "string. Never turn the spoken name into a slug. Call get_endpoint_details on the",
    "endpoint the node targets, read the permitted set for that field, and copy the",
    "exact string. If what the user named is not in the set, say so BEFORE writing",
    "anything — name what you were about to use and list what the service permits.",
    "Do not answer from memory: a model added last week is absent from what you know",
    "and present in the schema.",
    "validate_workflow does NOT check this. It checks structure and field names, not",
    "whether a value is one the service accepts — so a DAG can validate cleanly and",
    "still be refused on its first run. Never call a workflow \"tested\" on that basis.",
    "",
    "== WHEN YOU ARE DONE ==",
