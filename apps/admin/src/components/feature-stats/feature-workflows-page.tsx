...
import { cmpValues, fmtCount, fmtUsd, nextSort, type Sort } from "@/lib/feature-stats-format";
import {
...
/**
 * Workflow — every workflow ONE feature runs, scored across all client brands.
 *
 * Shared by every `/feature-stats/<slug>/workflows` route rather than copied per
 * feature: the table and its Edit-with-AI panel are one question asked of a
 * different subject, and a second copy is how two features come to score their
 * workflows two different ways.
 *
 ...
 * the outreach volume, plus the catalogue as a fourth source. Every cell prints
 * a served field; nothing here divides.
 */
export function FeatureWorkflowsPage({ featureSlug }: { featureSlug: string }) {
  ...
  const replies = useQuery({
    queryKey: ["crossOrgWorkflowCost", featureSlug, "positiveReply"],
    queryFn: () => getCrossOrgWorkflowCostPerOutcome(featureSlug, "positiveReply"),
    ...pollOptionsSlower,
  });

  const visits = useQuery({
    queryKey: ["crossOrgWorkflowCost", featureSlug, "websiteVisit"],
    queryFn: () => getCrossOrgWorkflowCostPerOutcome(featureSlug, "websiteVisit"),
    ...pollOptionsSlower,
  });

  const outreach = useQuery({
    queryKey: ["crossOrgWorkflowOutreach", featureSlug],
    queryFn: () => getCrossOrgWorkflowOutreach(featureSlug),
    ...pollOptionsSlower,
  });
  ...
  const catalogueQuery = useQuery({
    queryKey: ["workflows", featureSlug],
    queryFn: () => listWorkflows({ featureSlug }),
    ...pollOptionsSlower,
  });
  ...
  const aiContext = useMemo(
    () => ({
      ...
      featureSlug,
      workflows: catalogue,
      instructions: workflowCatalogueInstructions(featureSlug, catalogue),
    }),
    [catalogue, featureSlug],
  );
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
      ...
      <WorkflowAiPanel
        ...
        context={aiContext}
        // Chat thread key. Not a workflow UUID — the panel spans every workflow
        // of the feature, so one thread belongs to the page, and it is scoped by
        // feature so two features never share a conversation.
        sessionKey={`feature-stats:${featureSlug}`}
      />
    </div>
  );
}
